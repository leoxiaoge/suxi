...
class Ajax extends Base
{
    ...
    public function quickEdit(){
        ...
        if ($table == '') return returnJson(1,'缺少表名');
        if ($field == '') return returnJson(1,'缺少字段名');
        if ($id == '') return returnJson(1,'缺少主键值');
    ...
    public function tpost(){
        ...
        if($data[$data['field']]){
           $data['update_date'] = date('Y-m-d H:i:s',time());
            ...
            return returnJson(0,'修改成功',$data['gourl']?$data['gourl']:"");
             // $this->success('修改成功',$data['gourl']?$data['gourl']:"");  
        }else{
            $data['create_date'] = date('Y-m-d H:i:s',time());
          if($this->getbase->getadd($table,$data)){
            return returnJson(0,'添加成功',$data['gourl']?$data['gourl']:"");
            } 
        }
            ...
            if($this->request->isAJax()){
               $data = $this->request->param();
                if(model('Base')->getadd(decode($data['table']),$data)){
                     return returnJson(0,'添加成功',$data['returnurl']);
                }else{
                     return returnJson(1);
                } 
            }
