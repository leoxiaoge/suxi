...
class User extends PublicBase
{
    ...
    public function comment(){
        ...
        if(!cache('comment_self_'.$limitpage)){
             $info = $this->getbase->getall('selfdb_comment',['limit'=>"{$limitpage},10",'order'=>'time desc']);
             cache('comment_self_'.$limitpage,$info);
        }else{
             $info =cache('comment_self_'.$limitpage);
        }
       
        // show($this->getbase->getLastSql());
        // $info = $this->getbase->getall('selfdb_comment',['limit'=>10]);
        // foreach ($info as &$v) {
        //     $v['time'] = date('Y-m-d').' '.date('H:i:s',strtotime($v['time']));
        //     $v['img'] = "https://www.qiaolibeilang.com".get_file_path($v['img']);
        ...
        // $time =  date('Y-m-d ').rand(1,12).':'.rand(0,60);
        // $phone = $phoneprex[rand(0,28)]."****".rand(1234,9876);
        // $info = [];
        // $choosed_comment = [];
        // for ($i=0; $i < 5; $i++) { 
        //     $info[$i]['img'] = $this->c_rand_img(rand(1,4));
        //     $info[$i]['time'] = $time;
        //     $data = $this->getcomment();
        //     $choosed_comment[] = $data['number'];
        //     $info[$i]['comment'] = $data['comment'];
        //     $info[$i]['phone'] = $phone;
        // }
     ...
     * [c_rand_img 随机生成图片]
     * @Author   Jerry
     * @DateTime 2017-09-21T15:28:53+0800
     * @Example  eg:
     * @return   [type]                   [description]
     */
    public function c_rand_img($count=1){
        $img = [];
        for ($i=0; $i < $count; $i++) { 
            $img[] = "http://www.qiaolibeilang.com/public/static/images/pinjia/".rand(1,18).".jpg";
        }
        return $img;
    ...
    public function login()
    {
        ...
        //如果没有则进行入库 返回用户id
        $db = Db::name('users_wx')->insert($datau);
        $userId = Db::name('users_wx')->getLastInsID();
        if ($userId) {
            $datas['wxid']      = $userId;
            $db = Db::name('users')->insert($datas);
            if ($db) {
                $array = array('uid'=>$userId);
                return  returnJson(0,'success','',$array);
            }else{
        ...
        foreach ($orders as $k => $v) {
            ...
            $goodsinfo = $this->getbase->getall('goods',['where'=>['id'=>['in',trim($v['good_id'])]],'field'=>'picture']);
                foreach ($goodsinfo as  $vi) {
                    $orders[$k]['pictures'][]='https://www.qiaolibeilang.com'.get_file_path($vi['picture']);
                }
            ##是否评价
    ...
    //                 $array[$v['id']]['stars'] = floor($stars/4);
    //                 $array[$v['id']]['o_type'] = 2;
    ...
    //         }
    //         //获取用户下单的产品信息
    //         $g_id = array_filter(explode(',', $v['good_id']));//产品id
    //         $num = array_filter(explode(',', $v['good_num']));//产品数量
    //         $list = array();
    //         $g_num = 0;
    //         $i = 0;
    //         //循环查询产品详情
    ...
    public function ajax_address(){
        ...
        if($type == 1 and $id == 0){
            $where = 'parentno = 0';
        }elseif($type == 2 and $id > 0){
            $where = 'parentno = '.$id;
        }else{
            return  returnJson(1,'参数错误','');
        }
        $data = Db::name('prov_city_area')->where($where)->field('areano,areaname')->select();
     ...
     * [index 小程序用户查看地址]
     * @Author   wb
     * @describe  eg:提交用户信息进行验证后入库
     * @return  修改成功后的数据信息
     */
    public function u_address(){
        //查看单个地址信息
        $id    = intval(input('post.id'));
        $data = Db::name('address')->where('id='.$id.' and stat=0')->field('address_info,u_name,u_phone,id,sex,la,lo,address')->find();
