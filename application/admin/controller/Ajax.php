...
public function saveconfig(){
    if($this->request->isAJax()){
        ...
        cache('system_setting',$this->getbase->getall('system_setting',['order'=>'id asc']));
        return returnJson(0);
...
public function save_group(){
        if($this->request->isAJax()){
            ...
            if(!$data['group_name']) return returnJson(1,'组名不能为空');
            if($data['question_id']){
                //修改
                $question_id = $data['question_id'];
                unset($data['question_id']);
                model('base')->getedit('users_group',['where'=>['question_id'=>$question_id]],$data);
                return returnJson(0,'修改成功',url('admin/user/group'));
            }else{
                //新加
               if(model('base')->getone('users_group',['where'=>["group_name"=>"{$data['group_name']}"]]))  return returnJson(1,'组名已存在');
                    if(model('base')->getadd('users_group',$data)){
                    return returnJson(0,'添加成功',url('admin/user/group'));
                }else{
...
public function save_nv_menu_index(){
        if($this->request->isAJax()){
            $data = $this->request->param();
            if(!$data['title'])  return returnJson(1,'导航名不能为空');
            if($data['id']){
                //修改
                $id = $data['id'];
                unset($data['id']);
