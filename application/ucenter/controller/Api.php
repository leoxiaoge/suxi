...
class Api extends ApiBase
{
    ...
    public function adminlogin(){
        ...
        if(is_array($re)&&!empty($re)){
            ...
            if($re['password']==encode_pwd($password['password'],$re['salt'])){
                 parent::Auth('admin')->creatAdmin($re);
                 //hook('loginSuccess',$re);##登陆成功勾子
                 $data['url'] = input('gourl');
                 // show($data);
                 returnJson(0,'',$data['url']?$data['url']:turl('admin/index/index'),$data);
            }else{
                returnJson(1,'密码错误','');
            }
        }else{
    ...
    */
    public function logo()
    {
        ...
        if(is_array($re)&&!empty($re)){
            ...
            if($re['password']==encode_pwd($password['password'],$re['salt'])){
                 ...
                 returnJson(0,'',$data['gourl']?$data['url']:"/",$data);
            }else{
                returnJson(1,'密码错误','');
            }
        }else{
    ...
    public function adminLoginOut(){
        if($this->request->isAJax()){
            parent::Auth('admin')->delAdmin();
            returnJson(0,'退出成功',url('ucenter/admin/login'));
      }
    }
